/**
 * AppPalavras.js
 * Aplicativo para estudo de PALAVRAS
...
class AppPalavras extends _AppBase {
  ...
  extrairItens(texto) {
    return PalavrasModel.extrair(texto);
  }

  selecionarAleatorias(lista) {
    return PalavrasModel.selecionarAleatorias(lista, this.getMaxItens());
  }
    ...
    if (this.state.get().passo === 1) {
      appDiv.innerHTML = `
                <div class="bg-white rounded-2xl shadow-xl p-8">
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">1️⃣ Digite ou cole o texto</h2>
                    <textarea id="textoInput" rows="8" class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-purple-500 focus:outline-none font-mono" placeholder="Cole seu texto aqui. O programa vai extrair todas as palavras."></textarea>
                    <button id="btnProcessar" class="mt-4 bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition">Extrair Palavras →</button>
                </div>
            `;
    ...
    appDiv.innerHTML = `
            <div class="bg-white rounded-2xl shadow-xl p-8">
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <p class="text-sm text-gray-500 mt-2">🔊 O idioma selecionado será usado para ler as palavras em voz alta.</p>
                </div>
                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                    <label class="flex items-center cursor-pointer">
                        ...
                        <span class="text-gray-700 font-bold">🔊 Falar automaticamente cada palavra</span>
                    </label>
                    <p class="text-sm text-gray-500 mt-2 ml-8">🎤 Se ativado, cada palavra será pronunciada automaticamente. Você também pode clicar no botão de áudio durante o estudo.</p>
                </div>
                
                <div class="bg-blue-50 p-4 rounded-lg mb-6">
                    <p class="text-blue-800">✅ Total de palavras encontradas: <strong>${this.state.getTotalItens()}</strong></p>
                    <p class="text-blue-800">✅ Palavras selecionadas para estudo: <strong>${maxItens}</strong></p>
                    <p class="text-blue-800">📌 (Máximo de ${maxItens} palavras)</p>
                </div>
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">⏱️ Tempo entre palavras (segundos):</label>
  ...
  renderizarEstudo() {
    ...
    appDiv.innerHTML = `
            <div class="bg-white rounded-2xl shadow-xl p-8">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">3️⃣ Estudando Palavras...</h2>
                <div class="text-center mb-4" id="progresso">Palavra 1 de ${this.state.getTotalItens()}</div>
                <div class="bg-gradient-to-br from-purple-600 to-blue-500 rounded-2xl p-12 mb-6">
                    <div class="text-white text-5xl md:text-6xl font-bold text-center break-words" id="itemAtual">${this.state.getItemAtual()?.toUpperCase() || 'Pronto?'}</div>
    ...
    appDiv.innerHTML = `
            <div class="bg-white rounded-2xl shadow-xl p-8">
                ...
                <div class="flex flex-wrap gap-4 justify-center">
                    <button id="btnVerItens" class="bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 transition">📖 Ver Palavras</button>
                ...
                <div id="revisaoArea" class="hidden mt-6">
                    <h3 class="font-bold text-gray-700 mb-3">📖 Todas as palavras estudadas (na ordem):</h3>
  ...
  registrarSessaoEstudo() {
    if (typeof ProgressService !== 'undefined') {
      ...
      if (window.DEBUG_MODE) {
        console.log(
          `📊 Progresso registrado: sessão de palavras - ${this.state.getTotalItens()} palavras`
        );
      }
    }
  }
